...
const AccountProfile = ({user, btnTitle}:Props) => {
  ...
  const pathname = usePathname();
    const form = useForm({
        ...
        //type of UserValidation because we have to submit type 
        //the profile_photo: z.string().url().nonempty(),
    //name: z.string().min(3, {message: 'Minimum 3 Char'}).max(30),
    //username: z.string().min(3, {message: 'Minimum 3 Char'}).max(30),
    //etc, in lib/validations/user.ts
          ...
          render={({ field }) => (
            <FormItem className="flex items-center gap-4">
              ...
              <FormControl className="flex-1 text-base-semibold text-gray-200">
                <Input type="file" accept="image/*" placeholder="Upload a Phoyo"
